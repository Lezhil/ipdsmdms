...
<script  type="text/javascript">
	...
	$(".page-content").ready(function(){			
		...
		$('#MDMSideBarContents,#newConnectionId,#datamodification').addClass('start active ,selected');
				...
				<div class="col-md-12">
					...
					<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption"><i class="fa fa-globe"></i>New Meter Connection</div>
							<div class="tools">
						...
						<div class="portlet-body">
									...
									<form:form action="./addNewMtrConnectionAMI"  class="form-inline" role="form" modelAttribute="newConnectionMeterMaster" commandName="newConnectionMeterMaster" method="post" id="dataModificationId" >		  
													
													<h5><span style="color: #6e6750; font-size: medium;font-weight: normal;">Please Enter Surveyed Sdocode and MeterNo </span></h5>
													<hr>
													<table>
													
													
													<tr><td>
													<label>RdngMonth<form:input path="rdngmonth"  id="rdngmonth" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder=""  ></form:input></label>
													</td><td>
													<label>AccNo<form:input path="accno"  id="accno" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Enter ACCNO" maxlength="13" name="accno" ></form:input></label>
														</td>
														 <td>
													<label>SDOCODE<form:input path="master.sdocode"  id="sdocode" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Enter SDOCODE"  minlength="7" maxlength="7" ></form:input></label>
													</td> 
														
													<td>
													<label>MeterNo<form:input path="metrno"  id="meterno" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Enter Meterno ie Surveyed"  maxlength="10" onchange="return getMtrSurveyData(this.value);"></form:input></label>
													</td>
												
													
													<%-- <td>
													<label>ModemNo<form:input path="modem_no"  id="modemno" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="XXXXXXXXXX"  maxlength="10"></form:input></label>
													</td> --%>
											...
											<tr>
													<td><label>Zone
													<form:select path="master.zone"  id="zoneId" class="form-control "  onchange="getCircleByZone(this.value);">
													...
													<option value="0">Select Division</option>
													 ...
													 <c:forEach items="${division}" var="element">
														<option value="${element}">${element}</option>
													</c:forEach>
													</form:select>
													</label>
													...
													<c:forEach items="${sdoname}" var="element">
														<option value="${element}">${element}</option>
													</c:forEach> 
													</form:select>
													</label>
													...
													<td><label>Remark
									   					<form:textarea path="master.remarks" type="text" class="form-control"  name="remarks" id="remarks" maxlength="200"></form:textarea>
									   				 </label></td>
									        ...
									        <label>Supply Type<form:select  path="master.supplytype" name="supplyType" class="form-control" id="supplyType" >
                                             ...
                                             <form:option value="LT">LT</form:option>
													...
													<label>Industry Type<form:input path="master.industrytype"  id="industrytype" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="" maxlength="50"></form:input></label>
                                                       ...
                                                       <td>
													<label>KNO<form:input path="master.kno"  id="kno" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="XXXXXXXXXX"  maxlength="13" ></form:input></label>
													</td>
                                                       </tr>	
                                                       <tr>
                                                       <td><label>Tariff Code
									   					<form:input path="master.tariffcode" type="text" class="form-control"  name="tariffCode" id="tariffCode" maxlength="10"></form:input>
									   				 </label></td>
									   				 
									   				 <td><label>C Status
									   					<form:input path="master.consumerstatus" type="text" class="form-control"  name="consumerstatus" id="consumerstatus" maxlength="10"></form:input>
									   				 </label></td>
...
function myClearFunction() {
    document.getElementById("dataModificationId").reset();
}
...
function onEnter(accNum)
{
	 ...
	 /* if(accNum.match(regex))
	  ...
	  //$('#dataModificationId').attr('action','./getConnectionDataAMR').submit();
}


function getMtrSurveyData(metrno)
{
	//210463039019
	var sitecode=$("#sdocode").val();
	//alert("sitecode--"+sitecode.length);
	if(sitecode.length!=7){
		bootbox.alert("Please Enter 7 digit Sdocode")
		document.getElementById("dataModificationId").reset();
		return false;
	}
	...
	//sitecode='2104640';
		$.ajax({
		url:'http://220.227.2.202:7070/bsmartsurvey/getMeterDataByMTRNO?meterno='+metrno+'&sitecode='+sitecode,
		type:'GET',
    	dataType:'json',
    	asynch:false,
    	cache:false,
    	success:function(obj)
		{
    		...
    		if(data!=null && data!="")
			{    		
    		getLocationDetails(data[0],metrno)
    		$("#accno").val(data[2]);
    		$("#kno").val(data[1]);
    		$("#name").val(data[3]);
    		$("#address1").val(data[4]);
    		$("#mf").val(data[24]);
    			...
    			{
    			swal("Entered MeterNo Not Present in Survey Table ");
    			document.getElementById("dataModificationId").reset();
    			return false;
    			}
		}
...
function getLocationDetails(sdocode,meterNo)
{
	$.ajax({
		url:'./getLocationDetailsBySdocode',
		data:{sdocode:sdocode,meterNo:meterNo},
		type:'GET',
    	dataType:'json',
    	asynch:false,
    	cache:false,
    	success:function(response)
    		...
    		if(response.length>0 )
    		{
	    		...
	    		$("#zoneId").val(loc[0]);
	    		$("#circleId").val(loc[1]);
	    		$("#divisionId").val(loc[2]);
	    		$("#sdonameId").val(loc[3]);
    		}
    		else
    			{
    			/* bootbox.alert("MeterNo Already Exists"); */
    			swal("MeterNo "  +meterNo+ " Already Exists");
    			document.getElementById("dataModificationId").reset();
    			return false;
    			}
			}
