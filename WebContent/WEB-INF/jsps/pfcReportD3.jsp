...
$(".page-content").ready(
		function() {
			...
			TableEditable.init();
			FormComponents.init();
 			loadSearchAndFilter('sample_reportd3');
		 $('#eaId,#eaWiseReport,#pfcReportD3').addClass('start active ,selected');
...
function view() {
   ...
   $('#imageee').show();
	$('#sample_reportd3').dataTable().fnClearTable();
	$.ajax({
		...
		success:function(res){
			...
			if (res.length==0 || res.length == null) {
				...
				$.each(res,function(i,v){
					html+="<tr><td>"+(i+1)+"</td><td>"+v[7]+"</td><td>"+v[1]+"</td><td>"+v[2]+"</td><td>"+v[3]+"</td><td>"+v[4]+"</td><td>"+v[5]+"</td></tr>";
				});
			//	$('#sample_reportd3').dataTable().fnClearTable();
				clearTabledataContent('sample_reportd3');
				$("#complaintsid").html(html);
				loadSearchAndFilter('sample_reportd3');
				//$('#sample_reportd3').dataTable().fnClearTable();
...
<div class="page-content">
               <div class="portlet box blue">
                   ...
                   <div class="caption">
                   <i class="fa fa-bars"></i>PFC Report-D3
                   </div>
                   </div>
       ...
       <table style="border-collapse: separate; border-spacing: 8px;">  
               ...
               <select class="form-control input-large" name="scheme" id="scheme"  onchange="showtown(this.value);" >
             ...
             <input type="text" class="form-control from"  id="period" style="width:200px;" 
								onchange="dateformat(this.value);"		name="period" placeholder="Select Report Month" style="cursor: pointer"> 
             <%--   <select class="form-control input-large" name="period"  id="period" onchange="dateformat(this.value);">
              ...
              <!--  <div class="row"><br>
               <div class="col-md-4">
               <button class="btn green pull-left" id="" type="button" onclick="view('sample_reportd3','Table details')" style="position:relative; left:240%">View Report</button>
               <a href="#" id="excelExport" class="btn green pull-right" type="button"  onclick="tableToExcel('sample_reportd3', 'Comsumer Complaints')" style="position:relative; left:200%">Export to Excel</a>
               </div>
               </div> -->
                ...
                <div class="row"><br>
               <div class="col-md-2">
                <button class="btn blue pull-left" id="viewrpt" type="button" onclick="view('sample_reportd3','Table details')">View Report</button>
              ...
              <div><label style="color: #000000; background-color:#DCDCDC; width:100%; padding-top: 20px; float: left; text-align: center;"  data-toggle="modal">Consumer Complaints redressal report
				 </label>
			   </div>
			   ...
			   <div><label style="color: #000000; background-color:#DCDCDC; width:100%; padding-top: 10px; float: left; text-align: center;"  data-toggle="modal">Format:D3
				 </label>
			   </div>
       	  ...
       	  <div class="table-toolbar">
					<div class="btn-group pull-right" style="margin-top: 10px;">
							...
							<ul class="dropdown-menu pull-right">
								...
								<li><a href="#" id="" 
                                    onclick="exportPDF('sample_reportd3','Consumer Complaints redressal report')">Export to PDF</a></li>
								<li><a href="#" id="excelExport"
									onclick="exportToExcelMethod('sample_reportd3', 'Consumer Complaints redressal report')">Export
										to Excel</a></li>
							</ul>
						</div>
					</div>

                       <table class="table table-striped table-hover table-bordered" id="sample_reportd3">
                              <thead>
                                     <tr>
                                         <th>Sl.No</th> 
                                         <th>Name of Town</th>
                                         <th>Complaints pending <br>from previous <br>period</th>
                                         <th>Complaints registered<br> in current period</th>
                                         <th>Total pending<br> complaints</th>
                                         <th>complaints closed</th>
                                         <th>Complaints pending <br>period(Average)HH:MM</th>
                                     </tr>
                              </thead>
 ...
 function dateformat(param)
 {
	   ...
	   var monthVal=moment().month(monthNo-2).format('MMM');
	...
	function exportPDF()
	{
		 ...
		 var period= $("#period").val();
  		...
  		//window.open("./PFCreportD3PDF/"+scheme+"/"+town+"/"+period+"/"+state+"/"+discom+"/"+month+"/"+ieperiod)	
  		
  		window.location.href=("./PFCreportD3PDF?scheme="+scheme+"&town="+town+"&period="+period+"&state="+state+"&discom="+discom+"&month="+month+"&ieperiod="+ieperiod+"&townname="+townname);
	}
	...
	function exportToExcelMethod()
	{
		 ...
		 var per=period;
  		...
  		window.open("./exportToExcelPFCReportD3Data?scheme="+scheme+"&town="+town+"&period="+period+"&ieperiod="+ieperiod+"&endMonth="+endMonth);
	}
