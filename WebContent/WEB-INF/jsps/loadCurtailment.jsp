...
$( document ).ready(function() {
	...
	TableManaged.init();
	FormComponents.init();	
	$('#MDASSideBarContents,#meterOper,#ondempro,#loadCurtailmentLi').addClass('start active ,selected');
		...
		<div class="col-md-12">
			<div class="portlet box blue">
				<div class="portlet-title">
					<div class="caption">
						<i class="fa fa-edit"></i>Create Load Curtailment Job
					</div>
				</div>
				<div class="portlet-body">
					...
					<div class="row">
						...
						<div class="form-group col-md-3">
							<label>Load Curtailment State</label>
							<div class="input-group">
								<span class="input-group-addon"> <i class="fa fa-indent"></i>
								</span> <select class="form-control select2me" id="lc_state"
									name="lc_state">
						...
						<div class="form-group col-md-3">
							<!-- <label>Power Limit Normal (kW)</label> -->
							<label>Sanction Load (kW)</label>
							<div class="input-group">
								<span class="input-group-addon"> <i class="fa fa-flash"></i>
								</span> <input type="text" class="form-control " id="lc_p_limit"
									name="lc_p_limit" placeholder="Power Limit Normal"></input>
							</div>
						</div>
						<div class="form-group col-md-3">
							<!-- <label>Limit Over Duration (S)</label> -->
							<label>LC Limit (S)</label>
							<div class="input-group">
								<span class="input-group-addon"> <i class="fa fa-flash"></i>
								</span> <input type="text" class="form-control " id="lc_limit_od"
					...
					<div class="row">
						<div class="form-group col-md-3">
							<label>Limit Under Duration (s)</label>
							<div class="input-group">
								<span class="input-group-addon"> <i class="fa fa-flash"></i>
								</span> <input type="text" class="form-control " id="lc_limit_ud"
									name="lc_limit_ud" placeholder="Limit Under Duration"></input>
							</div>
						</div>
						<div class="form-group col-md-3">
							<label>Alert Period (s)</label>
							<div class="input-group">
								<span class="input-group-addon"> <i class="fa fa-clock-o"></i>
								</span> <input type="text" class="form-control " id="alert_period"
									name="alert_period" placeholder="Alert Period"></input>
							</div>
						</div>
						<div class="form-group col-md-3">
							<!-- <label>Lockout Period (s)</label> -->
							<label>LS Period of Re-Connection(S)</label>
							<div class="input-group">
								<span class="input-group-addon"> <i class="fa fa-clock-o"></i>
								</span> <input type="text" class="form-control " id="lockout_prd"
									name="lockout_prd" placeholder="Lockout Period"></input>
							</div>
						</div>
						<div class="form-group col-md-3">
							<!-- <label>Lockout Max Counter </label> -->
							<label>Signal Retrieve </label>
							<div class="input-group">
								<span class="input-group-addon"> <i
									class="fa fa-dashboard"></i>
								</span> <input type="text" class="form-control " id="lockout_m_counter"
							...
							<table class="table table-striped table-bordered table-hover "
								...
								<thead>
									...
									<!-- <tr>
										<th>Type</th>
										<th>State</th>
										<th>Power Limit Normal (mW)</th>
										<th>Limit Over Duration (mW)</th>
										<th>Limit Under Duration (mW)</th>
										<th>Alert Period (mS)</th>
										<th>Lockout Period (mS)</th>
										<th>Lockout Max Counter (mS)</th>
...
<script>
	...
	function validateFields() {
		...
		var lc_state = $("#lc_state").val();
		var lc_p_limit = $("#lc_p_limit").val();
		var lc_limit_od = $("#lc_limit_od").val();
		var lc_limit_ud = $("#lc_limit_ud").val();
		var alert_period = $("#alert_period").val();
		var lockout_prd = $("#lockout_prd").val();
		var lockout_m_counter = $("#lockout_m_counter").val();
		...
		if (!$.isNumeric(lc_p_limit)) {
			bootbox.alert("Power Limit Should be a Number.");
			return false;
		}
		if (!$.isNumeric(lc_limit_od)) {
			bootbox.alert("Limit Over Duration Should be a Number.");
			return false;
		}
		if (!$.isNumeric(lc_limit_ud)) {
			bootbox.alert("Limit Under Duration Should be a Number.");
			return false;
		}
		if (!$.isNumeric(alert_period)) {
			bootbox.alert("Alert Period Should be a Number.");
			return false;
		}
		if (!$.isNumeric(lockout_prd)) {
			bootbox.alert("Lockout Period Should be a Number.");
			return false;
		}
		if (!$.isNumeric(lockout_m_counter)) {
			bootbox.alert("Lockout Max Counter Should be a Number.");
			return false;
		}

		lc_p_limit = parseInt(lc_p_limit)*1000000;
		lc_limit_od = parseInt(lc_limit_od)*1000;
		lc_limit_ud = parseInt(lc_limit_ud)*1000;
		alert_period = parseInt(alert_period)*1000;
		lockout_prd = parseInt(lockout_prd)*1000;
	...
	function resetFields() {
		...
		$("#job_name").val("");
		$("#lc_state").val("ENABLED").trigger("change");
		$("#lc_p_limit").val("");
		$("#lc_limit_od").val("");
		$("#lc_limit_ud").val("");
		$("#alert_period").val("");
	...
	function meterjoblist() {
		...
		$
				.ajax({
					url : "./meterjoblist/LOAD_CURTAILMENT",
					type : "GET",
					success : function(response) {
		...
		$.ajax({
			...
			success : function(response) {
				...
				for (var i = 0; i < t2.length; i++) {
					if(t2[i].type=='LOAD_CURTAILMENT'){
						var normarlL=(t2[i].powerLimitNormal==null?"":(parseInt(t2[i].powerLimitNormal)/1000000));
						var limitOd=(t2[i].powerLimitMinOverThresholdDuration==null?"":(parseInt(t2[i].powerLimitMinOverThresholdDuration)/1000));
						var limitUd=(t2[i].powerLimitMinUnderThresholdDuration==null?"":(parseInt(t2[i].powerLimitMinUnderThresholdDuration)/1000));
						var alertP=(t2[i].alertPeriod==null?"":(parseInt(t2[i].alertPeriod)/1000));
						var lockoutP=(t2[i].lockoutPeriod==null?"":(parseInt(t2[i].lockoutPeriod)/1000));
						
						html += "<tr>"+
						"<td>Type</td><td>" + t2[i].type + "</td></tr>"+
						"<td>State</td><td>"+ t2[i].loadCurtailmentState + "</td></tr>"+
						"<td>Power Limit Normal (kW)</td><td>"+ normarlL + "</td></tr>"+
						"<td>Limit Over Duration (S)</td><td>"+ limitOd + "</td></tr>"+
						"<td>Limit Under Duration (S)</td><td>"+ limitUd + "</td></tr>"+
						"<td>Alert Period (S)</td><td>"+ alertP + "</td></tr>"+
						"<td>Lockout Period (S)</td><td>"+ lockoutP + "</td></tr>"+
						"<td>Lockout Max Counter</td><td>"+ t2[i].lockoutMaxCounter + "</td></tr>";
					}
