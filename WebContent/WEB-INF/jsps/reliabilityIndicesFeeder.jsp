          ...
          if (this.value.match(/[^0-9]/g)) {
              this.value = this.value.replace(/[^0-9^-]/g, '');
	   	     ...
	   	     $("#fromDateMonthly").hide();
	   	   	...
	   	   	$('#MDMSideBarContents,#reportsId,#reliabilityIndicesFeeder').addClass('start active ,selected');
 ...
 */
  ...
  /* getting substations and feeder based on subdivision */
	  ...
	  $.ajax({
          ...
          data : {
        	  ...
        	  town : town
                  ...
                  success : function(response) {
                      var html = '';
                      html += "<select id='substationId' name='substationId' onchange='showfdrIdNameBySubdivAndSSname(this.value)' class='form-control select2me input-medium' type='text'><option value=''>Select Sub-Division</option><option value='%'>ALL</option>";
                      for (var i = 0; i < response.length; i++) {
                          //var response=response1[i];
		...
		/* $.ajax({
				url : './showSSnameBySubdiv' + '/' + subdivision,
			    	type:'GET',
			    	dataType:'json',
			    	success:function(response)
			    	{
  ...
  function showfdrIdNameBySubdivAndSSname(parentid)
	{
	  ...
	  $.ajax({
          url : './showfdrIdNameBySubdivAndSSname',
          type : 'GET',
          dataType : 'json',
          asynch : false,
          cache : false,
          data : {
        	  ...
        	  sdoCode : sdoCode, 
                  ...
                  success : function(response) {
                      var html = '';
                      html += "<select id='substationId' name='substationId'  class='form-control select2me input-medium' type='text'><option value=''>Select Sub-Division</option><option value='%'>ALL</option>";
                      for (var i = 0; i < response.length; i++) {
                          //var response=response1[i];
		...
		<div class="page-content" >
		    <div class="portlet box blue "  id="boxid">
			    <div class="portlet-title">
					<div class="caption"><i class="fa fa-reorder"></i>Reliability Indices (Feeder)</div>
					<div class="tools">
			...
			<table >
			   <tbody>
					...
					<tr >
						...
						<th class="block" id=''>Sub-Division&nbsp;:</th>
								<th id="subdivTd">
									<select
						...
						<th class="block" id='reportType'>Report Type &nbsp;:</th>
							<th>
								<div class="radio-inline" id = "reportPeriod">
	                			 <label><input type="radio" name="reportType" value= "singleFeederDT"  checked="checked" onchange="handleClick(this.value);">Single Feeder</label>
	            	           	</div>
	                       
		                       	<div class="radio-inline" style="padding-left:5px" >
		                			 <label><input type="radio" name="reportType" value= "multipleFeederDT"  onchange="handleClick(this.value);" >Multiple Feeder</label>
		                       	</div>
							</th>	
					...
					<th  id=subStationName >Sub-Station&nbsp;:</th>
							<th id="subStationDIV">
								<select	class="form-control select2me input-medium" id="subStationList"
								name="subStation" onclick="showfdrIdNameBySubdivAndSSname(this.value);">
									<option value="">Select Substation</option>
									
						     	</select>
							</th>
							<th  id=feederName >Feeder&nbsp;:</th>
									...
									<div id="excelExportDiv2">
										...
										<ul class="dropdown-menu pull-right">
												...
												<li><a href="#" id="excelExport2" 
													onclick="tableToExcel2('sample_3', 'Reliability Indices (Feeder)')">Export
									 				to Excel</a></li>
										</ul>
	...
	$.ajax({
		...
		success:function(res){
			...
			$.each(res,function(i,v){
				html+="<tr><td>"+(i+1)+"</td><td>"+fromdate+"</td><td>"+v[0]+"</td><td>"+v[1]+"</td><td>"+v[2]+"</td><td>"+v[3]+"</td><td>"+v[4]+"</td><td>"+v[5]+"</td>"+
				"<td>"+v[8]+"</td><td>"+v[9]+"</td><td>"+v[10]+"</td><td>"+v[11]+"</td></tr>";
			});
			//$("#listID").empty().
