...
@Entity
@Table(name = "LOAD_SURVEY",  schema = "METER_DATA",uniqueConstraints = { @UniqueConstraint(columnNames = { "READ_TIME","METER_NUMBER" }) })
@NamedQueries({
	...
	/*@NamedQuery(name = "AmrLoadEntity.getRecords", query = "select  a from AmrLoadEntity a where myKey.meterNumber=:meterno and to_char(a.myKey.readTime, 'YYYY-MM-DD' ) =:fileDate ORDER BY a.myKey.readTime")*/
	
	
	
	
	
	@NamedQuery(name = "AmrLoadEntity.findEntityByUniId", query = "select a from AmrLoadEntity a where a.myKey.meterNumber=:meterNumber and a.myKey.readTime=:readTime"),
	
	@NamedQuery(name = "AmrLoadEntity.findEntityByLId", query = "select a.id from AmrLoadEntity a where a.myKey.meterNumber=:meterNumber and a.myKey.readTime=:readTime"),
	
	@NamedQuery(name = "AmrLoadEntity.getRecords", query = "select  a from AmrLoadEntity a where myKey.meterNumber=:meterno and to_date(to_char(a.myKey.readTime, 'YYYY-MM-DD' ),'yyyy-MM-dd') BETWEEN to_date(:beforeDate,'yyyy-MM-dd') and to_date(:onePluseDateString,'yyyy-MM-dd') ORDER BY a.myKey.readTime ASC"),
	@NamedQuery(name = "AmrLoadEntity.findEntityById", query = "select a from AmrLoadEntity a where id = :id")
	
	/*@NamedQuery(name = "AmrLoadEntity.getRecords", query = "select  a from AmrLoadEntity a where myKey.meterNumber=:meterno and to_char(a.myKey.readTime, 'YYYY-MM-DD' ) <=:fileDate and to_char(a.myKey.readTime, 'YYYY-MM-DD' ) >=:fileDate-'4' ORDER BY a.myKey.readTime")*/

})
public class AmrLoadEntity implements Comparable<AmrLoadEntity> {
...
/*	public int getId() {
	...
	public AmrLoadEntity() {

	}

	@Override
	public int compareTo(AmrLoadEntity amrLoadEntity) {
